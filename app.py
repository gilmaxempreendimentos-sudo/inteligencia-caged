...
st.set_page_config(
    page_title="Inteligência Salarial | Executivo",
    page_icon="📊",
    layout="wide",
...
st.markdown("""
    ...
    /* Fundo levemente acinzentado para destacar os cards brancos */
    .stApp { background-color: #F8FAFC; }
    
    /* Estilização Premium dos Cards de Métricas */
...
# --- TRADUTOR DE CÓDIGOS IBGE (APENAS NOMES COMPLETOS PARA EVITAR TRADUÇÃO) ---
mapa_estados_ibge = {
    ...
    '52': 'Goiás', '53': 'Distrito Federal', '99': 'Não Informado'
}

# Converte a coluna para texto e aplica a tradução
df['estado'] = df['estado'].astype(str).map(mapa_estados_ibge).fillna(df['estado'])

# 4. BARRA LATERAL (FILTROS)
st.sidebar.title("🎯 Estratégia")
st.sidebar.markdown("Configure os parâmetros de mercado.")

estados_disponiveis = sorted(df['estado'].unique())

# Ajustando o padrão para o nome completo
estado_padrao = ["Rio de Janeiro"] if "Rio de Janeiro" in estados_disponiveis else [estados_disponiveis[0]]

estado_sel = st.sidebar.multiselect(
    "Localidades (Estado)", 
    estados_disponiveis, 
    default=estado_padrao
)

min_amostra = st.sidebar.slider("Confiabilidade Estatística (Amostra)", 1, 100, 3)

# 5. CONTEÚDO PRINCIPAL
st.title("Inteligência Competitiva Salarial")
...
if not df_filtrado.empty:
    ...
    # --- GRÁFICO 1: COMPARAÇÃO ENTRE SETORES ---
    ...
    fig_setores.update_layout(
        font_family="Inter",
        height=max(400, len(df_cargo) * 45), 
        showlegend=False,
        plot_bgcolor='rgba(0,0,0,0)',
        xaxis=dict(range=[0, df_cargo['media_salarial'].max() * 1.25], showgrid=False, zeroline=False),
    ...
    # --- GRÁFICO 2: CURVA DE SÉNIORIDADE DENTRO DO SETOR ---
    ...
    df_senioridade = pd.DataFrame({
        ...
        "Salário": [
            dados_setor['junior_p25'], dados_setor['pleno_p50'], 
            dados_setor['senior_p75'], dados_setor['master_p90']
        ]
    })

    fig_senioridade = px.bar(
        df_senioridade, x='Nível', y='Salário', 
        color='Salário', color_continuous_scale='Teal', text='Salário'
    )
    ...
    fig_senioridade.update_layout(
        font_family="Inter",
        showlegend=False, height=450, margin=dict(t=30),
        plot_bgcolor='rgba(0,0,0,0)',
        yaxis=dict(range=[0, df_senioridade['Salário'].max() * 1.3], showgrid=True, gridcolor='#E2E8F0', title=""),
        xaxis=dict(tickfont=dict(size=14), title="")
    )
